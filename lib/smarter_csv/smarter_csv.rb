module SmarterCSV
  ...
  def SmarterCSV.parse(input, options={}, &block)   # first parameter: filename or input object with readline method
    default_options = {:col_sep => ',' , :row_sep => $/ , :quote_char => '"', :force_simple_split => false , :verbose => false ,
      :remove_empty_values => true, :remove_zero_values => false , :remove_values_matching => nil , :remove_empty_hashes => true , :strip_whitespace => true,
      :convert_values_to_numeric => true, :strip_chars_from_headers => nil , :user_provided_headers => nil , :headers_in_file => true,
      :comment_regexp => /^#/, :chunk_size => nil , :key_mapping_hash => nil , :downcase_header => true, :strings_as_keys => false, :file_encoding => 'utf-8',
    ...
    begin
      ...
      if options[:row_sep] == :auto
        options[:row_sep] =  SmarterCSV.guess_line_ending_from_data( input )
      end
      $/ = options[:row_sep]
      ...
      if options[:headers_in_file]        # extract the header line
        # process the header line in the CSV file..
        # the first line of a CSV file contains the header .. it might be commented out, so we need to read it anyhow
        input_array = input.split(options[:row_sep])
        header = input_array.shift
        line_count += 1
        header = header.gsub(options[:strip_chars_from_headers], '') if options[:strip_chars_from_headers]
        if (header =~ %r{#{options[:quote_char]}}) and (! options[:force_simple_split])
          file_headerA = CSV.parse( header, csv_options ).flatten.collect!{|x| x.nil? ? '' : x} # to deal with nil values from CSV.parse
      ...
      input_array.each_with_index do |line, index|
        ...
        if use_chunks
          ...
          if chunk.size >= chunk_size || (index+1==num_lines)    # if chunk if full, or EOF reached
            # do something with the chunk
            if block_given?
              yield chunk  # do something with the hashes in the chunk in the block
            else
...
#  end
  ...
  def guess_line_ending_from_data(input)
    row_sep = nil # "\r\n"

    ["\r", "\n", "\r\n"].each do |c|
      row_sep = c if input.split(c).size > 1
    end
    puts "row_sep found #{row_sep}"
  end
end
